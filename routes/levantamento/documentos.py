...
@router.get("/matriculas/{matricula_id}/requerimento-averbacao-casamento-html", response_class=HTMLResponse)
def get_requerimento_averbacao_casamento_matricula_direta_html(
    matricula_id: int,
    cliente_id: Optional[int] = Query(None),
    data_celebracao: Optional[str] = Query(None),
    regime_bens: Optional[str] = Query(None),
    ...
    """Gera o Requerimento de Averbação de Casamento diretamente da matrícula, sem necessidade de levantamento técnico ativo."""
    try:
        from services.documentacao.cartorio_generator import CartorioReportGenerator
        params_dict = {
            "data_celebracao": data_celebracao,
